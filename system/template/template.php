...
class AppTemplate
{
    	 ...
    	 //对图片路径的修复
	        ...
	        {        
	        	if(trim($layout_array[3][$k])!='/') //有关联数据
	        	{
	        		...
	        		$adv = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."adv where tmpl='".app_conf("TEMPLATE")."' and adv_id='".$adv_id."' and rel_table = '".$rel_array[1][0]."' and rel_id = ".intval($rel_array[3][0])." and is_effect = 1".$city_condition);
	        	}	
	        	else
	        	{
	        		$adv = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."adv where tmpl='".app_conf("TEMPLATE")."' and adv_id='".$adv_id."' and is_effect = 1".$city_condition);      	
	        	}
	        	if($adv['city_ids']!='')
	        	{
    ...
    function _compile_if_tag($tag_args, $elseif = false)
    {
        ...
        $tokens = $match[0];
        // make sure we have balanced 1u8j4t8g4r0f9b4n9re4e
