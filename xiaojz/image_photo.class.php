...
class Images{
	...
	function __construct($inputName,$allowType="",$allowSize="",$saveDir="",$isRename=true){
        ...
        if(isset($_GET['photo_id'])){
            ...
            $sql = "SELECT photo_big_img FROM `xiaojz`.`xiaojz_photo` WHERE photo_id='{$_GET['photo_id']}';";
       ...
       $info  = $this->getImageInfo($image);
         if($info !== false) {
            ...
            if ($suofang==1) {
                ...
                $thumbImg01 = imagecreatetruecolor(226,152);
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],226,152,$_POST['w'],$_POST['h']);

                if(isset($_GET['photo_id'])){
                    require('db_connect.php');
                    $xiaojz_mysqli->query("UPDATE `xiaojz`.`xiaojz_photo` SET `photo_small_img` = '{$thumbname01}' WHERE `xiaojz_photo`.`photo_id` ='{$_GET['photo_id']}';");
                    header('Location: index.php?r=administrator/photo/show');
                }
