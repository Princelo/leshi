       ...
       $info  = $this->getImageInfo($image);
         if($info !== false) {
            ...
            if ($suofang==1) {
                ...
                if(isset($_COOKIE['user_name'])){
                    $sql = "UPDATE `xiaojz`.`xiaojz_players` SET `players_small_img` = '{$thumbname01}' WHERE `xiaojz_players`.`players_user_name` ='{$_COOKIE['user_name']}';";
                    include('db_connect.php');
                    $xiaojz_mysqli->query($sql);
                    $sql = "UPDATE `xiaojz`.`xiaojz_players` SET `is_cut`='1' WHERE `xiaojz_players`.`players_user_name` = '{$_COOKIE['user_name']}';";
                    $xiaojz_mysqli->query($sql);
