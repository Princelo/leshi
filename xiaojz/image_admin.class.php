<?php
class Images{
	public $inputName;                 //控件名
	public $allowType = array(
				'image/gif','image/jpg','image/jpeg','image/png','image/x-png','image/pjpeg'
	);	                           //上传类型
	public $allowSize = 2097152;	//限制大小
	//public $saveDir = "./uploads/";      //保存目录
    public $saveDir = "";
	public $isRename = true;               //是否重命名，默认为true
	public $errID = 0;                     //错误代码，默认为0
	public $errMsg = "";                   //错误信息
	...
	function __construct($inputName,$allowType="",$allowSize="",$saveDir="",$isRename=true){
        ...
        if(isset($_GET['players_id'])){
            ...
            $result = $xiaojz_mysqli->query("SELECT players_big_img FROM `xiaojz`.`xiaojz_players` WHERE players_id='{$_GET['players_id']}';")->fetch_array();
    ...
    function getImageInfo($img) {
        /*if(!isset($img)){
            require("db_connect.php");
            $sql = "SELECT players_big_img FROM `xiaojz`.`xiaojz_players` WHERE players_id='{$_GET['players_id']}';";
            $result = $xiaojz_mysqli->query($sql)->fetch_array();
            $img = $result['players_big_img'];
        }*/
    ...
    function thumb($image,$is_save=true,$suofang=0,$type='',$maxWidth=412,$maxHeight=547,$interlace=true){
        // 获取原图信息
       $info  = $this->getImageInfo($image);
         if($info !== false) {
			...
			$length = ($type != $_type ? $length+2 : $length+1);
            //裁剪
            if ($suofang==1) {
				
				$thumbname01 = substr_replace($image,"_thumb.".$type,$length);		//大头像
				//$thumbname02 = substr_replace($image,"02.".$type,$length);		//小头像
				$imageFun($thumbImg,$thumbname01,99);
				//$imageFun($thumbImg,$thumbname02,99);

                $thumbImg01 = imagecreatetruecolor(225,290);
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],225,290,$_POST['w'],$_POST['h']);

                if(isset($_GET['players_id'])){
                    $sql = "UPDATE `xiaojz`.`xiaojz_players` SET `players_small_img` = '{$thumbname01}' WHERE `xiaojz_players`.`players_id` ='{$_GET['players_id']}';";
                    include('db_connect.php');
                    $xiaojz_mysqli->query($sql);
                    $sql = "UPDATE `xiaojz`.`xiaojz_players` SET `is_cut`='1' WHERE `xiaojz_players`.`players_id` = '{$_GET['players_id']}';";
                    $xiaojz_mysqli->query($sql);
                    header('Location: index.php?r=administrator/players/show');
                }
				...
				//$imageFun($thumbImg02,$thumbname02,99);
//				unlink($image);
				...
				if($is_save == false){											//缩略图覆盖原图，缩略图的路径还是原图路径
					$imageFun($thumbImg,$image,99);
				}else{
					$thumbname03 = substr_replace($image,"03.".$type,$length);	//缩略图与原图同时存在，
					$imageFun($thumbImg,$thumbname03,99);
