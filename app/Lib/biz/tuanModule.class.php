...
class tuanModule extends BizBaseModule
{
	...
	public function deal_coupon()
	{
		...
		$coupon_list = $GLOBALS['db']->getAll("select c.* from ".DB_PREFIX."deal_coupon as c left join ".DB_PREFIX."deal_location_link as l on c.deal_id = l.deal_id where l.location_id in (".implode(",",$s_account_info['location_ids']).") and c.deal_id = ".$deal_id." and c.is_valid = 1 and c.is_delete = 0 order by c.id desc limit ".$limit);
	
		$GLOBALS['tmpl']->assign('coupon_list',$coupon_list);
		
		$coupon_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_coupon as c left join ".DB_PREFIX."deal_location_link as l on c.deal_id = l.deal_id where l.location_id in (".implode(",",$s_account_info['location_ids']).") and c.deal_id = ".$deal_id." and c.is_valid = 1 and c.is_delete = 0");
