...
class Page  {
   ...
   private function get_page_link($url,$page)
    ...
    public function show() {
        ...
        if ($upRow>0){
            $upPage="<a class='upPage' href='".$this->get_page_link($url,$upRow)."'>".$this->config['prev']."</a>";
        }else{
            $upPage="";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a class='downPage' href='".$this->get_page_link($url,$downRow)."'>".$this->config['next']."</a>";
        }else{
            $downPage="";
        }
        // << < > >>
        if($nowCoolPage == 1){
            ...
            $prePage = "<a class='prePage' href='".$this->get_page_link($url,$preRow)."' >上".$this->rollPage."页</a>";
            $theFirst = "<a class='theFirst' href='".$this->get_page_link($url,1)."' >".$this->config['first']."</a>";
        }
        if($nowCoolPage == $this->coolPages){
            ...
            $nextPage = "<a class='nextPage' href='".$this->get_page_link($url,$nextRow)."' >下".$this->rollPage."页</a>";
            $theEnd = "<a class='theEnd' href='".$this->get_page_link($url,$theEndRow)."' >".$this->config['last']."</a>";
        }
        // 1 2 3 4 5
        $linkPage = "";
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
                    $linkPage .= "&nbsp;<a class='linkPage' href='".$this->get_page_link($url,$page)."'>&nbsp;".$page."&nbsp;</a>";
                }else{
                    break;
                }
            }else{
