...
class cartModule extends ShopBaseModule
{
	...
	{	
		...
		{						
			...
			$buy_type = $GLOBALS['db']->getOne("select buy_type from ".DB_PREFIX."deal where id='".$id."'");
			$cart_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_cart where session_id='".$session_id."' and user_id = ".intval($GLOBALS['user_info']['id'])." and verify_code = '".$verify_code."' and buy_type = {$buy_type}");
