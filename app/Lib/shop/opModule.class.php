...
class opModule extends ShopBaseModule
{
	...
	function op_dp_setbest($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","setbest"))
		{
			...
			else
			{
				...
				send_user_msg("商户点评被推荐",$reason,intval($GLOBALS['user_info']['id']),$dp['user_id'],get_gmtime(),0,true,true);
			}			
			$result['status'] = 1;			
		}	
		else
	...
	function op_dp_del($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","del"))
		{
			...
			if($change==1||$o_reason!="")
			send_user_msg("商户点评被删除",$reason,intval($GLOBALS['user_info']['id']),$dp['user_id'],get_gmtime(),0,true,true);		
	...
	function op_dp_replydel($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","replydel"))
		{
			...
			if($change==1||$o_reason!="")
			send_user_msg($reason,$reason,intval($GLOBALS['user_info']['id']),$dp_reply['user_id'],get_gmtime(),0,true,true);		
			$result['status'] = 1;			
		}	
		else
		{
	...
	function op_topic_del($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("topic","del"))
		{
			...
			if($change==1||$o_reason!="")
			send_user_msg($reason,$reason,intval($GLOBALS['user_info']['id']),$topic['user_id'],get_gmtime(),0,true,true);		
			$result['status'] = 1;			
		}	
		else
		{
	...
	function op_topic_replydel($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($change==1||$o_reason!="")
			send_user_msg($reason,$reason,intval($GLOBALS['user_info']['id']),$topic_reply['user_id'],get_gmtime(),0,true,true);		
			$result['status'] = 1;			
		}	
		else
		{
