...
class mobile_paymentModule extends ShopBaseModule
{
    public function pay()
    {
        ...
        if($payment_notice)
        {
            if($payment_notice['is_paid'] == 0)
            {
                ...
                $GLOBALS['tmpl']->display("mobile/mobile_payment_pay.html");
            }
            else
            {
                $order = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_order where id = ".$payment_notice['order_id']);
    ...
    public function done()
    {
        ...
        $GLOBALS['tmpl']->display("mobile/mobile_payment_done.html");
    }
