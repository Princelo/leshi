...
class mobile_uc_centerModule extends ShopBaseModule
{
...
//		$user_info['from'] = $address['area1'].$address['area2'];
    ...
    public function index()
    {
        ...
        $user_data = $GLOBALS['db']->getALL("select point,money,level_id from ".DB_PREFIX."user where id =".$user_id." LIMIT 1");
        $user_data = $user_data[0];
        $level = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."user_level where id = ".intval($user_data['level_id']));
        $GLOBALS['tmpl']->assign('level', $level);
        $GLOBALS['tmpl']->assign('point', intval($user_data['point']));
        $GLOBALS['tmpl']->assign('money', format_price($user_data['money']));
        ...
        $GLOBALS['tmpl']->display('mobile/mobile_member.html');
