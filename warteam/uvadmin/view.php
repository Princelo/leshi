...
<body>
    <form action="teamchange.php" id="myform" name="myform" method="get" onsubmit="validate();">
    战队名称：<input id="teamname" name="teamname" <?php if(isset($_GET["teamname"])&&$_GET["teamname"]!="")echo "value=\"{$_GET["teamname"]}\" ";?>/> <br />
    选择比赛时区：&nbsp;&nbsp;A<input id="warzone" value="a" type="radio" <?php if(isset($_GET["warzone"])&& $_GET["warzone"]=="za") echo " checked=\"checked\" ";?>name="warzone" />&nbsp;&nbsp;&nbsp;
    B<input <?php if(isset($_GET["warzone"])&& $_GET["warzone"]=="zb") echo " checked=\"checked\" ";?>type="radio" id="warzone" name="warzone" value="b" /><br>
    ...
    <input type="submit" onclick="return validate();" value="<?php echo isset($_GET["tid"])?"修改队伍":"增加队伍"?>">
        <?php if(isset($_GET["tid"])) echo "<input name=\"tid\" value={$_GET["tid"]} type=\"hidden\" /><a href=\"index.php\">返回</a>"; ?>
