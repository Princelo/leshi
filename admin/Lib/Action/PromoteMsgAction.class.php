...
class PromoteMsgAction extends CommonAction{
	...
	public function gen_deal_mail()
	{
		...
		$rs = M("Deal")->where("is_delete = 0 and id in (".$id.") and is_shop = 0")->findAll();
		if($rs)
		{
			$content = get_deal_mail_content($rs);
	...
	public function gen_deal_sms()
	{
		...
		$rs = M("Deal")->where("is_delete = 0 and id = ".$id." and is_shop = 0")->find();
		if($rs)
		{
	...
	//开始验证
		...
		$msg_data['send_time'] = trim($_REQUEST['send_time'])==''?get_gmtime():to_timespan($_REQUEST['send_time']);
