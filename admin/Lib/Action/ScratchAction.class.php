    ...
    public function send()
    {
        ...
        if($msg_item)
        {
            ...
            else
            {
                ...
                if($msg_item['is_html'])
                    $mail_content = $msg_item['content']."<br />".sprintf(app_conf("UNSUBSCRIBE_MAIL_TIP"),app_conf("SHOP_TITLE"),"<a href='".get_domain().APP_ROOT."/subscribe.php?act=unsubscribe&code=".base64_encode($msg_item['dest'])."'>".$GLOBALS['lang']['CANCEN_EMAIL']."</a>");
                else
                    $mail_content = $msg_item['content']." \n ".sprintf(app_conf("UNSUBSCRIBE_MAIL_TIP"),app_conf("SHOP_TITLE"),$GLOBALS['lang']['CANCEN_EMAIL'].get_domain().APP_ROOT."/subscribe.php?act=unsubscribe&code=".base64_encode($msg_item['dest']));
